...
public class DepositFoundsController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<BaseResponse> depositFounds(@PathVariable String id, @RequestBody DepositFoundsCommand command) {

        try{
            command.setId(id);
            commandDispatcher.send(command);
            return new ResponseEntity<>(new OpenAccountResponse("Deposit founds request completed successfully"), HttpStatus.OK);
